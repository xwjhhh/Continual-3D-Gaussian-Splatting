...
// CL-Splats: Added active_gaussian_mask for local optimization
__global__ void computeCov2DCUDA(int P,
	...
	float* dL_dcov,
	bool antialiasing,
	// CL-Splats: Local optimization parameter
	const bool* active_gaussian_mask)  // [P] mask for active Gaussians (nullptr = all active)
{
	auto idx = cg::this_grid().thread_rank();
	if (idx >= P || !(radii[idx] > 0))
		return;

	// CL-Splats: Skip non-active Gaussians
	if (active_gaussian_mask != nullptr && !active_gaussian_mask[idx])
		return;
...
// CL-Splats: Added active_gaussian_mask and tile_mask for local optimization
template <uint32_t C>
...
renderCUDA(
	...
	const bool* __restrict__ active_gaussian_mask,  // [P] mask for active Gaussians (nullptr = all active)
	const bool* __restrict__ tile_mask              // [tiles] mask for active tiles (nullptr = all active)
)
{
	...
	// CL-Splats: Check tile mask - skip entire tile if not active
	const uint32_t tile_id = block.group_index().y * horizontal_blocks + block.group_index().x;
	if (tile_mask != nullptr && !tile_mask[tile_id])
	{
		// This tile is not active, skip all computation
		return;
	}
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// CL-Splats: Check if this Gaussian is active for gradient updates
			const bool is_active = (active_gaussian_mask == nullptr) || active_gaussian_mask[global_id];
			...
			for (int ch = 0; ch < C; ch++)
			{
				...
				// CL-Splats: Only update gradients for active Gaussians
				if (is_active)
...
void BACKWARD::preprocess(
	...
	bool antialiasing,
	// CL-Splats: Local optimization parameter
	const bool* active_gaussian_mask)
{
	...
	computeCov2DCUDA << <(P + 255) / 256, 256 >> > (
		...
		dL_dcov3D,
		antialiasing,
		// CL-Splats: Pass active_gaussian_mask
	...
	const bool* tile_mask)
{
	renderCUDA<NUM_CHANNELS> << <grid, block >> >(
		...
		dL_dcolors,
		dL_dinvdepths,
		// CL-Splats: Pass local optimization parameters
		active_gaussian_mask,
		tile_mask
		);
}
