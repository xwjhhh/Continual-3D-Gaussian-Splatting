...
def first_timestamp_subset(dataset, eps=1e-6):
    raw_dataset = getattr(dataset, "dataset", None)
    if raw_dataset is None or len(raw_dataset) == 0:
        return dataset
    times = [float(getattr(item, "time", 0.0)) for item in raw_dataset]
    first_time = min(times)
    indices = [idx for idx, value in enumerate(times) if abs(value - first_time) <= eps]
    if not indices or len(indices) == len(raw_dataset):
        return dataset
    print(
        f"coarse_first_timestep_only: selected {len(indices)}/{len(raw_dataset)} "
        f"training cameras at time={first_time:.6f}"
    )
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    if stage == "coarse" and getattr(opt, "coarse_first_timestep_only", False):
        train_cams = first_timestamp_subset(train_cams)
...
if __name__ == "__main__":
    ...
    if args.configs:
        ...
        try:
            ...
            config = {
                key: value
                for key, value in runpy.run_path(args.configs).items()
