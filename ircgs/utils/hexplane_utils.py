import itertools
from typing import Optional, Collection, Iterable, Sequence
...
def grid_sample_wrapper(
    ...
    interp = interp.view(bsz, feature_dim, n).transpose(-1, -2)
...
class HexPlaneField(nn.Module):
    ...
    def set_aabb(self, xyz_max, xyz_min):
        ...
        if int(xyz_max_t.numel()) != int(xyz_min_t.numel()):
            raise ValueError(
                "HexPlaneField.set_aabb expects xyz_max and xyz_min to have the same shape, "
                f"got {tuple(xyz_max_t.shape)} vs {tuple(xyz_min_t.shape)}."
            )
